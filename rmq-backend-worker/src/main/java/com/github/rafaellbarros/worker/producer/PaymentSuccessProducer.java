...
public class PaymentSuccessProducer {
    ...
    public void generateResponse(String successMessage, @Nullable String originalTraceId) {
        try {
            Message message = MessageBuilder
                    .withBody(successMessage.getBytes(StandardCharsets.UTF_8))
            ...
            log.info("Payment success response sent [OriginalTraceId: {}] - Message: {}",
                    originalTraceId,
                    truncate(successMessage));
        } catch (Exception e) {
            log.error("Failed to send success response [TraceId: {}]", getCurrentTraceId(), e);
